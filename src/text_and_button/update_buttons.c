...
static void update_button(sfRenderWindow *window, sfSprite *sprite)
{
    ...
    if (is_on_sprite(sprite, mouse_pos)) {
        if (sfMouse_isButtonPressed(sfMouseLeft))
            sfSprite_setTextureRect(sprite, (sfIntRect){0, 40, 200, 20});
        else
            sfSprite_setTextureRect(sprite, (sfIntRect){0, 20, 200, 20});
    } else
        sfSprite_setTextureRect(sprite, (sfIntRect){0, 0, 200, 20});
}
